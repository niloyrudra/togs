...
const CommentModal = ({ navigation, refEle, isVisible, onClose, eventId }) => {
    ...
    React.useEffect( () => {
        if( showModal ) {
            ...
            setCommentData( comments.filter(item => { if( item.eventId == eventId ) return item } )[0].data )
        }
        else {
            Animated.timing( modelAnimatedValue, {
                toValue: 0,
    ...
    const onSubmit = async (data) => {
        ...
        await onAddComments( eventId, data )
